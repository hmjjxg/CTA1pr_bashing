---
title: "Oleic Acid on ScCTA1pr and ScCTT1pr"
author: "JY"
date: "2/4/2024"
...
names(files) <- c("Rep1-H2O2-OA-Ctrl","Rep2-H2O2-OA-Ctrl","Rep3-OA-OA-Ctrl")
...
names(tr_files) <- c("Rep1-H2O2-OA-Ctrl","Rep2-H2O2-OA-Ctrl","Rep3-OA-OA-Ctrl")
...
### CTA1 induction during stress
Get the subset of no Pi data
```{r}
oa.dat <- data %>% 
  filter(treat == "0.2%_Oleic_Acid") %>%  # , HB_strain != "yH298"
  select(Time = Group, Sample, Plate, replicate = Replicate, genotype, median, HB_strain) %>%
  mutate(Genotype = fct_recode(genotype, `wt` = "wt", `ScCTA1pr (200-1kb)` = "ScCTA1pr",  `ScCTT1pr (200-1kb)` = "ScCTT1pr")) %>% 
...
ctrl.dat <- data %>% 
  filter(treat == "ctrl") %>%  # , HB_strain != "yH298"
  select(Time = Group, Sample, Plate, replicate = Replicate, genotype, median, HB_strain) %>%
  mutate(Genotype = fct_recode(genotype, `wt` = "wt", `ScCTA1pr (200-1kb)` = "ScCTA1pr",  `ScCTT1pr (200-1kb)` = "ScCTT1pr")) %>% 
  mutate(new_median =median/1000) %>% 
  mutate(new_time = as.numeric(gsub("min","",Time)))%>% 
...
names(genotype_color) <- c("wt", "ScCTA1pr (200-1kb)", "ScCTT1pr (200-1kb)")
...
#ggsave("output/Rev2468_0Pi.png", width = 6.3, height = 6)

h2o2.dat %>%
  filter(!is.na(new_median)) %>% 
  ggplot(aes(x = new_time, y = new_median, color = Genotype)) + p.timecourse +
  scale_color_manual(values= genotype_color, limits = names(genotype_color))
#ggsave("output/Rev2468_2mMH2O2.png", width = 6.3, height = 6)
...
```
### Take out the yH298 strain
```{r}
oa.dat <- data %>% 
  filter(treat == "0.2%_Oleic_Acid", HB_strain != "yH298") %>%  # 
  select(Time = Group, Sample, Plate, replicate = Replicate, genotype, median, HB_strain) %>%
  mutate(Genotype = fct_recode(genotype, `CgCTA1pr` = "wt", `ScCTA1pr (200-1kb)` = "ScCTA1pr",  `ScCTT1pr (200-1kb)` = "ScCTT1pr")) %>% 
  mutate(new_median =median/1000) %>% 
  mutate(new_time = as.numeric(gsub("min","",Time)))%>% 
  arrange(new_time, genotype, replicate)
...
oa.dat %>%
  ...
  ggplot(aes(x = new_time, y = new_median, color = Genotype)) + p.timecourse +
  scale_color_manual(values= genotype_color, limits = names(genotype_color)) +
  labs(x = "0.2% Oleic Acid, Time (min)")
#ggsave("output/OA.png", width = 6.3, height = 6)
...
#ggsave("output/2mMH2O2.png", width = 6.3, height = 6)
...
#ggsave("output/Ctrl.png", width = 6.3, height = 6)
```

### only plot the Sc data
```{r}
sc.h2o2.dat <- data %>% 
  filter(treat == "2mMH2O2", genotype %in% c("ScCTA1pr","ScCTT1pr")) %>%  # 
  select(Time = Group, Sample, Plate, replicate = Replicate, genotype, median, HB_strain) %>%
  mutate(Genotype = fct_recode(genotype,`ScCTA1pr (200-1kb)` = "ScCTA1pr",  `ScCTT1pr (200-1kb)` = "ScCTT1pr")) %>% 
  mutate(new_median =median/1000) %>% 
  mutate(new_time = as.numeric(gsub("min","",Time)))%>% 
  arrange(new_time, genotype, replicate)

sc.oa.dat <- data %>% 
  filter(treat == "0.2%_Oleic_Acid", genotype %in% c("ScCTA1pr","ScCTT1pr")) %>%  # 
  select(Time = Group, Sample, Plate, replicate = Replicate, genotype, median, HB_strain) %>%
  mutate(Genotype = fct_recode(genotype,`ScCTA1pr (200-1kb)` = "ScCTA1pr",  `ScCTT1pr (200-1kb)` = "ScCTT1pr")) %>% 
  mutate(new_median =median/1000) %>% 
  mutate(new_time = as.numeric(gsub("min","",Time)))%>% 
...
sc.ctrl.dat <- data %>% 
  filter(treat == "ctrl", genotype %in% c("ScCTA1pr","ScCTT1pr")) %>%  # 
  select(Time = Group, Sample, Plate, replicate = Replicate, genotype, median, HB_strain) %>%
  mutate(Genotype = fct_recode(genotype,`ScCTA1pr (200-1kb)` = "ScCTA1pr",  `ScCTT1pr (200-1kb)` = "ScCTT1pr")) %>% 
  mutate(new_median =median/1000) %>% 
  mutate(new_time = as.numeric(gsub("min","",Time)))%>% 
...
genotype_color <- RColorBrewer::brewer.pal(8, "Dark2")[c(7, 2)] 
#genotype_color <- RColorBrewer::brewer.pal(8, "Dark2")[c(8, 7, 2, 3, 5, 1)] 
names(genotype_color) <- c("ScCTA1pr (200-1kb)", "ScCTT1pr (200-1kb)")
...
sc.h2o2.dat %>%
  ...
  scale_color_manual(values= genotype_color, limits = names(genotype_color)) +
  labs(x = "2mM H2O2, Time (min)")
#ggsave("output/Sc_H2O2.png", width = 6.3, height = 6)
...
sc.oa.dat %>%
  ...
  scale_color_manual(values= genotype_color, limits = names(genotype_color)) +
  labs(x = "0.2% Oleic Acid, Time (min)")
#ggsave("output/Sc_oa.png", width = 6.3, height = 6)
...
sc.ctrl.dat %>%
  ...
  scale_color_manual(values= genotype_color, limits = names(genotype_color)) +
  labs(x = "Control, Time (min)")
#ggsave("output/Sc_Ctrl.png", width = 6.3, height = 6)
