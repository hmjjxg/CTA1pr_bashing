...
oa.dat <- data %>% 
  ...
  mutate(Genotype = fct_recode(genotype, `wt` = "wt", `ScCTA1pr (1kb)` = "ScCTA1pr",  `ScCTT1pr (1kb)` = "ScCTT1pr")) %>% 
...
names(genotype_color) <- c("wt", "ScCTA1pr (1kb)", "ScCTT1pr (1kb)")
...
oa.dat <- data %>% 
  filter(treat == "0.2%_Oleic_Acid", HB_strain != "yH298") %>%  # 
  select(Time = Group, Sample, Plate, replicate = Replicate, genotype, median, HB_strain) %>%
  mutate(Genotype = fct_recode(genotype, `CgCTA1pr` = "wt", `ScCTA1pr (1kb)` = "ScCTA1pr",  `ScCTT1pr (1kb)` = "ScCTT1pr")) %>% 
  mutate(new_median =median/1000) %>% 
  mutate(new_time = as.numeric(gsub("min","",Time)))%>% 
  arrange(new_time, genotype, replicate)
...
sc.h2o2.dat <- data %>% 
  ...
  select(Time = Group, Sample, Plate, replicate = Replicate, genotype, median, HB_strain) %>%
  mutate(Genotype = fct_recode(genotype,`ScCTA1pr (1kb)` = "ScCTA1pr",  `ScCTT1pr (1kb)` = "ScCTT1pr")) %>% 
  mutate(new_median =median/1000) %>% 
  mutate(new_time = as.numeric(gsub("min","",Time)))%>% 
  arrange(new_time, genotype, replicate)
...
#genotype_color <- RColorBrewer::brewer.pal(8, "Dark2")[c(8, 7, 2, 3, 5, 1)] 
names(genotype_color) <- c("ScCTA1pr (1kb)", "ScCTT1pr (1kb)")
