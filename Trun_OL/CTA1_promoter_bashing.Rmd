...
```{r}
# recode the genotype
levels <- c(`WT` = "wt", `PC`= "rescue",`IR_8.10` = "10OL", `IR_6.8` = "8OL", `IR_4.6` = "6OL", `IR_2.4` = "4OL")
...
pi.ol.dat <- data.ol.pi %>% 
  ...
  mutate(dP_series = fct_recode(genotype, 'WT' = "wt", 'PC' = "rescue", 'dP4' = "10OL", 'dP3' = "8OL", 'dP2' = "6OL", 'dP1' = "4OL"))
...
full_palette <- brewer.pal(n_colors, "YlGnBu") # display.brewer.all()
...
pi.ol.dat %>%
  filter(!is.na(new_median), genotype != "rescue") %>% 
  ggplot(aes(x = new_time, y = new_median, color = dP_series)) + p.timecourse +
  scale_darker_ygb +
  labs(x = "Phosphate starvation, Time (min)")+
   theme(
    axis.title.x = element_blank(),
...
#ggsave("output/OL_0Pi_ygb.png", width = 3.7, height = 6)
...
#ggsave("output/OL_oxi_ygb.png", width = 3.7, height = 6)
...
```{r}
# recode the genotype
levels <- c(`WT` = "wt", `PC`= "rescue", `IR_8.10` = "8V", `IR_6.10` = "6V", `IR_4.10` = "4V", `IR_2.10` = "2V" )
levels.norescue <- c(`WT` = "wt",`IR_8.10` = "8V", `IR_6.10` = "6V", `IR_4.10` = "4V", `IR_2.10` = "2V" )
...
ggsave("output/TrunV_0Pi_ygb.png", width = 3.7, height = 6)
```
#### 2mM H2O2 Data
...
ggsave("output/TrunV_oxi_ygb.png", width = 3.7, height = 6)
```
...
```{r}
# add column genotype through Genotype name
rs.slope <- read.csv("input/rs_slope_rel.csv") %>% mutate(genotype = case_when(
  Genotype == "WT" ~ "wt",
  Genotype == "IS_2.4" ~ "RS1_24",
  Genotype == "IS_2.6" ~ "RS1_26",
  Genotype == "IS_2.8" ~ "RS1_28",
  Genotype == "IS_2.10" ~ "RS1_210",
))

TrunV.slope <- TrunV.slope %>%
  mutate(genotype = case_when(
    ...
    Genotype == "IR_6.10" ~ "6V",
    Genotype == "IR_8.10" ~ "8V",
    TRUE ~ as.character(Genotype) # Default case, retaining the original value
  ))

ol.slope <- ol.slope %>% mutate(genotype = case_when(
  Genotype == "WT" ~ "wt",
  Genotype == "PC" ~ "rescue",
  Genotype == "IR_2.4" ~ "4OL",
  Genotype == "IR_4.6" ~ "6OL",
  Genotype == "IR_6.8" ~ "8OL",
  Genotype == "IR_8.10" ~ "10OL", 
  TRUE ~ as.character(Genotype)
  ))
  

slope.rel <- bind_rows(TrunV.slope, ol.slope) %>% select(slope, rel_slope, Genotype, genotype, Treatment) %>%
             bind_rows(.,rs.slope) %>% 
...
# combine tP and dP variants for epistasis test (max slope & basal)
```{r}
OL.levels <- c("wt","4OL", "6OL", "8OL", "10OL")
TrunV.levels <- c("wt","2V", "4V", "6V", "8V")
RS.levels <- c("wt","RS1_24","RS1_26","RS1_28","RS1_210")



## 1. Max Slope (tP and dP series)
slope.rel.no_rp.pi <- slope.rel %>% filter(genotype %in% c(OL.levels, TrunV.levels), Treatment == "0Pi")
slope.rel.no_rp.oxi <- slope.rel %>% filter(genotype %in% c(OL.levels, TrunV.levels), Treatment == "2mMH2O2")

# variable selection for the lm
# - Pi:
full.model.pi.no_rp <- lm(rel_slope ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                             beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = slope.rel.no_rp.pi)
base.model.pi.no_rp <- lm(rel_slope ~ 1, data = slope.rel.no_rp.pi)

# 2mM H2O2:
full.model.oxi.no_rp <- lm(rel_slope ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                             beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = slope.rel.no_rp.oxi)
base.model.oxi.no_rp <- lm(rel_slope ~ 1, data = slope.rel.no_rp.oxi)

#choose a model by AIC in a stepwise algorithm
# forward
step(base.model.pi.no_rp, scope= list(upper=full.model.pi.no_rp,lower=~1),direction="forward")
step(base.model.oxi.no_rp, scope = list(upper = full.model.oxi.no_rp, lower =~1), direction = "forward")

# backward: all prefers full model
step(full.model.pi.no_rp, direction="backward") 
step(full.model.oxi.no_rp, direction = "backward") 

#the best forward and backward stepwise selected model: (better than full model)
# - Pi: (both fwd and bwd)
fit.pi.fwd <-   lm(rel_slope ~  beta_6.8 + beta_2.4 + beta_4.6 + beta_8.10 + beta_2.4:beta_4.6 +
                               beta_6.8:beta_8.10, data = slope.rel.no_rp.pi)
round(summary(fit.pi.fwd)$coef, 5)
pi.fwd <- glance(fit.pi.fwd) # AIC = -169, BIC = -150 , R square adj = 0.992

# 2mM h2o2:(both fwd and bwd = full model)
fit.oxi.fwd <- lm(rel_slope ~   beta_2.4 + beta_8.10 + beta_6.8 + beta_4.6 +  beta_6.8:beta_8.10 + beta_6.8:beta_4.6 + beta_2.4:beta_4.6, 
                  data = slope.rel.no_rp.oxi)
round(summary(fit.oxi.fwd)$coef, 5)
oxi.fwd <- glance(fit.oxi.fwd) # AIC = -118, BIC = -95 , R square adj = 0.81
...
fit.pi <- lm(rel_slope ~  beta_2.4 + beta_6.8 + beta_4.6 + beta_8.10 + 
               beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = slope.rel.no_rp.pi)
round(summary(fit.pi)$coef, 3)
pi <- glance(fit.pi) # AIC = -167, BIC = -145 , R square adj = 0.915



# 2. Basal
bas.rel.no_rp.pi <- bas.rel %>% filter(genotype %in% c(OL.levels, TrunV.levels), Treatment == "0Pi")
bas.rel.no_rp.oxi <- bas.rel %>% filter(genotype %in% c(OL.levels, TrunV.levels), Treatment == "2mMH2O2")
bas.rel.no_rp <- bas.rel %>% filter(genotype %in% c(OL.levels, TrunV.levels))
...
# - Pi:
full.model.pi <- lm(rel_base ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                 beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = bas.rel.no_rp.pi)
base.model.pi <- lm(rel_base ~ 1, data = bas.rel.no_rp.pi)

# 2mM H2O2:
full.model.oxi <- lm(rel_base ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                 beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = bas.rel.no_rp.oxi)
base.model.oxi <- lm(rel_base ~ 1, data = bas.rel.no_rp.oxi)


# all treatment:
full.model <- lm(rel_base ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = bas.rel.no_rp )
base.model <- lm(rel_base ~ 1, data = bas.rel.no_rp )
...
#the best forward stepwise selected model:
# - Pi: (fwd and bwd are very close, show both and have bin decide)
## fwd
fit.pi.fwd <- lm(rel_base ~  beta_8.10 + beta_4.6 + beta_6.8 + beta_4.6:beta_6.8, data = bas.rel.no_rp.pi)
pi.fwd <- glance(fit.pi.fwd) # AIC = -167, BIC = -152.43, R square adj = 0.95

## bwd
fit.pi.bwd <- lm(rel_base ~   beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 + beta_2.4:beta_4.6 + beta_4.6:beta_6.8, data = bas.rel.no_rp.pi)
round(summary(fit.pi.bwd)$coef, 5)
pi.bwd <- glance(fit.pi.bwd) # AIC = -171, BIC = -151.79, R square adj = 0.96

# 2mM h2o2: (choose full model)
fit.oxi.fwd <- lm(rel_base ~  beta_8.10 + beta_2.4 + beta_6.8 + beta_8.10:beta_6.8, data = bas.rel.no_rp.oxi)
oxi.fwd <- glance(fit.oxi.fwd)#  AIC = -154.47, BIC = -139, R square adj = 0.916

# two treatments: (choose full model)
fit.fwd <- lm(rel_base ~ beta_8.10 + beta_2.4 + beta_6.8 + beta_8.10:beta_6.8, data = bas.rel.no_rp)
fwd <- glance(fit.fwd) # AIC = -264.67, BIC = -245,48, R square adj = 0.909



# full model
# 2mM H2O2: (bwd selection)
fit.oxi <- bas.rel.no_rp.oxi %>% lm(rel_base ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                             beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = .)
round(summary(fit.oxi)$coef, 5)
oxi.full <- glance(fit.oxi) #  AIC = -242, BIC = -218, R square adj = 0.97

# combine - Pi and H2O2: (bwd selection)
fit.all <- bas.rel.no_rp  %>% lm(rel_base ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                             beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = .)
round(summary(fit.all)$coef, 5)
full <- glance(fit.all) #  AIC = -407, BIC = -378, R square adj = 0.96
...
# 3. Plateau
plateau.no_rp.pi <- plateau %>% filter(genotype %in% c(OL.levels, TrunV.levels), Treatment == "0Pi")
plateau.no_rp.oxi <- plateau %>% filter(genotype %in% c(OL.levels, TrunV.levels), Treatment == "2mMH2O2")


# variable selection for the lm (with reduction)
# - Pi:
full.model.pi <- lm(rel_red ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                             beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = plateau.no_rp.pi )
base.model.pi <- lm(rel_red ~ 1, data = plateau.no_rp.pi )

# 2mM H2O2:
full.model.oxi <- lm(rel_red ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                 beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = plateau.no_rp.oxi)
base.model.oxi <- lm(rel_red ~ 1, data = plateau.no_rp.oxi)
...
#the best forward and backward stepwise selected model: (better than full model)
# - Pi: (both fwd and bwd)
fit.pi.fwd <- lm(rel_red ~   beta_2.4 + beta_6.8 + beta_4.6 + beta_8.10 + beta_2.4:beta_4.6 + beta_6.8:beta_4.6, data = plateau.no_rp.pi)
round(summary(fit.pi.fwd)$coef, 5)
pi.fwd <- glance(fit.pi.fwd) # AIC = -124, BIC = -105,  R square adj = 0.875

# 2mM h2o2:
#fwd: (smaller AIC& BIC)
fit.oxi.fwd <- lm(rel_red ~   beta_2.4 + beta_8.10 + beta_6.8 + beta_4.6 + beta_2.4:beta_4.6, data = plateau.no_rp.oxi)
round(summary(fit.oxi.fwd)$coef, 5)
oxi.fwd <- glance(fit.oxi.fwd)#  AIC = -144, BIC = -126, R square adj = 0.83

#bwd
fit.pi.bwd <- lm(rel_red ~   beta_2.4 + beta_6.8 + beta_4.6 + beta_8.10 + beta_2.4:beta_4.6 + beta_6.8:beta_4.6, data = plateau.no_rp.pi)
round(summary(fit.pi.bwd)$coef, 5)
pi.bwd <- glance(fit.pi.bwd) # AIC = -124, BIC = -104,  R square adj = 0.875


# full model
# - Pi:
fit.pi <- plateau.no_rp.pi %>% lm(rel_red ~ beta_2.4 + beta_4.6 + beta_6.8 + beta_8.10 +
                             beta_2.4:beta_4.6 + beta_4.6:beta_6.8 + beta_6.8:beta_8.10, data = .)
round(summary(fit.pi)$coef, 5)
pi <- glance(fit.pi) # AIC = -124, BIC = -102,  R square adj = 0.876

```

#separately model three series for main effect
```{r}
OL.levels <- c("wt","4OL", "6OL", "8OL", "10OL")
TrunV.levels <- c("wt","2V", "4V", "6V", "8V")
RS.levels <- c("wt","RS1_24","RS1_26","RS1_28","RS1_210")
...
## basal
# - Pi:
bas.rel.ol.pi <- bas.rel %>% filter(genotype %in% OL.levels, Treatment == "0Pi")

fit.ol.pi <- lm(rel_base ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = bas.rel.ol.pi)
round(summary(fit.ol.pi)$coef, 5)


# 2mM h2o2:
bas.rel.ol.oxi <- bas.rel %>% filter(genotype %in% OL.levels, Treatment == "2mMH2O2")

fit.ol.oxi <- lm(rel_base ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = bas.rel.ol.oxi )
round(summary(fit.ol.oxi)$coef, 5)


# combined:
bas.rel.ol <- bas.rel %>% filter(genotype %in% OL.levels)
fit.ol <- lm(rel_base ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = bas.rel.ol)
round(summary(fit.ol)$coef, 5)


## plateau
plateau.rel.ol.pi <- plateau %>% filter(genotype %in% OL.levels, Treatment == "0Pi")
plateau.rel.ol.oxi <- plateau %>% filter(genotype %in% OL.levels, Treatment == "2mMH2O2")

# - Pi
fit.ol.pi <- lm(rel_red ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = plateau.rel.ol.pi )
round(summary(fit.ol.pi)$coef, 5)

# 2mM H2O2:
fit.ol.oxi <- lm(rel_red ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = plateau.rel.ol.oxi )
round(summary(fit.ol.oxi)$coef, 5)


## max slope
slope.rel.ol.pi <- slope.rel %>% filter(genotype %in% OL.levels, Treatment == "0Pi")
slope.rel.ol.oxi <- slope.rel %>% filter(genotype %in% OL.levels, Treatment == "2mMH2O2")

# - Pi
fit.ol.pi <- lm(rel_slope ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = slope.rel.ol.pi )
round(summary(fit.ol.pi)$coef, 5)

# 2mM H2O2:
fit.ol.oxi <- lm(rel_slope ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = slope.rel.ol.oxi )
round(summary(fit.ol.oxi)$coef, 5)


# Truancation variants
## basal
# - Pi:
bas.rel.pi <- bas.rel %>% filter(genotype %in% TrunV.levels, Treatment == "0Pi")

fit.pi <- lm(rel_base ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = bas.rel.pi)
round(summary(fit.pi)$coef, 5)


# 2mM h2o2:
bas.rel.oxi <- bas.rel %>% filter(genotype %in% TrunV.levels, Treatment == "2mMH2O2")

fit.oxi <- lm(rel_base ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = bas.rel.oxi )
round(summary(fit.oxi)$coef, 5)


# combined:
bas.all <- bas.rel %>% filter(genotype %in% TrunV.levels)
fit.bas <- lm(rel_base ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = bas.all)
round(summary(fit.bas)$coef, 5)


## plateau
plateau.rel.pi <- plateau %>% filter(genotype %in% TrunV.levels, Treatment == "0Pi")
plateau.rel.oxi <- plateau %>% filter(genotype %in% TrunV.levels, Treatment == "2mMH2O2")

# - Pi
fit.pi <- lm(rel_red ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = plateau.rel.pi )
round(summary(fit.pi)$coef, 5)

# 2mM H2O2:
fit.oxi <- lm(rel_red ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = plateau.rel.oxi )
round(summary(fit.oxi)$coef, 5)


## max slope
slope.rel.pi <- slope.rel %>% filter(genotype %in% TrunV.levels, Treatment == "0Pi")
slope.rel.oxi <- slope.rel %>% filter(genotype %in% TrunV.levels, Treatment == "2mMH2O2")

# - Pi
fit.pi <- lm(rel_slope ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = slope.rel.pi )
round(summary(fit.pi)$coef, 5)

# 2mM H2O2:
fit.oxi <- lm(rel_slope ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = slope.rel.oxi )
round(summary(fit.oxi)$coef, 5)
...
## basal
# - Pi:
bas.rel.pi <- bas.rel %>% filter(genotype %in% RS.levels, Treatment == "0Pi")
...
# 2mM h2o2:
bas.rel.oxi <- bas.rel %>% filter(genotype %in% RS.levels, Treatment == "2mMH2O2")
...
# combined:
bas.all <- bas.rel %>% filter(genotype %in% RS.levels)
fit.bas <- lm(rel_base ~   beta_2.4 + beta_4.6 +  beta_6.8 + beta_8.10, data = bas.all)
round(summary(fit.bas)$coef, 5)


## plateau
plateau.rel.pi <- plateau %>% filter(genotype %in% RS.levels, Treatment == "0Pi")
plateau.rel.oxi <- plateau %>% filter(genotype %in% RS.levels, Treatment == "2mMH2O2")
...
## max slope
slope.rel.pi <- slope.rel %>% filter(genotype %in% RS.levels, Treatment == "0Pi")
slope.rel.oxi <- slope.rel %>% filter(genotype %in% RS.levels, Treatment == "2mMH2O2")
...
# dPs
```{r}
dPs.levels <- c("wt", "10OL", "8OL", "6OL","4OL")


## Plateau loss
dPs.plateau <- plateau %>% filter(genotype %in% dPs.levels) %>% 
                 select(Genotype,Time,genotype,rel_ori,rel_red,Treatment) %>% 
                 mutate(loss_ori = 1 - rel_ori, loss_red = 1 - rel_red, genotype = factor(genotype, levels = dPs.levels)) 

## Slope loss
dPs.slope <- slope.rel %>% filter(genotype %in% dPs.levels) %>% 
               select(Genotype, rel_slope, Treatment, genotype) %>% 
               mutate(loss_slope = 1 - rel_slope, genotype = factor(genotype, levels = dPs.levels))



## Basal loss
new.bas <- bas.rel %>% filter(genotype %in% dPs.levels) %>% 
  select(Genotype, Time, rel_base, Treatment,genotype) %>% 
  mutate(loss_base = 1 - rel_base, genotype = factor(genotype, levels = dPs.levels))
...
dPs.plateau %>% 
 ...
 geom_bar_pattern(stat = "summary", fun = "mean",
  ...
  theme(strip.background = element_rect(colour="black", fill="white", size=1, linetype="solid"), 
        ...
        axis.title.x=element_blank())
#ggsave("output/dPs_plateau_loss.png", width = 10.16, height = 2)
...
dPs.slope %>% 
 ...
 geom_bar_pattern(stat = "summary", fun = "mean",
  ...
  theme(strip.background = element_rect(colour="black", fill="white", size=1, linetype="solid"), 
        legend.position= "none",
        axis.title.x=element_blank())
ggsave("output/dPs_slope_loss.png", width = 10.16, height = 2)
...
new.bas %>% 
 ...
 geom_bar_pattern(stat = "summary", fun = "mean",
  ...
  theme(strip.background = element_rect(colour="black", fill="white", size=1, linetype="solid"), 
        legend.position= "none",
        axis.title.x=element_blank())
#ggsave("output/base_loss_dPs.png", width = 10.16, height = 2)

```
# tPs
```{r}

tPs.levels <- c("wt", "8V", "6V", "4V","2V")


## Plateau loss
tPs.plateau <- plateau %>% filter(genotype %in% tPs.levels) %>% 
                 select(Genotype,Time,genotype,rel_ori,rel_red,Treatment) %>% 
                 mutate(loss_ori = 1 - rel_ori, loss_red = 1 - rel_red, genotype = factor(genotype, levels = tPs.levels)) 

## Slope loss
tPs.slope <- slope.rel %>% filter(genotype %in% tPs.levels) %>% 
               select(Genotype, rel_slope, Treatment, genotype) %>% 
               mutate(loss_slope = 1 - rel_slope, genotype = factor(genotype, levels = tPs.levels))



## Basal loss
new.bas <- bas.rel %>% filter(genotype %in% tPs.levels) %>% 
  select(Genotype, Time, rel_base, Treatment,genotype) %>% 
  mutate(loss_base = 1 - rel_base, genotype = factor(genotype, levels = tPs.levels))
...
tPs.plateau %>% 
 ...
 geom_bar_pattern(stat = "summary", fun = "mean",
  ...
  theme(strip.background = element_rect(colour="black", fill="white", size=1, linetype="solid"), 
        legend.position= c(0.02, 0.9),
        axis.title.x=element_blank())
ggsave("output/tPs_plateau_loss.png", width = 10.16, height = 2)
...
tPs.slope %>% 
 ...
 geom_bar_pattern(stat = "summary", fun = "mean",
  ...
  theme(strip.background = element_rect(colour="black", fill="white", size=1, linetype="solid"), 
        legend.position= "none",
        axis.title.x=element_blank())
ggsave("output/tPs_slope_loss.png", width = 10.16, height = 2)
...
new.bas %>% 
 ...
 geom_bar_pattern(stat = "summary", fun = "mean",
  ...
  theme(strip.background = element_rect(colour="black", fill="white", size=1, linetype="solid"), 
        legend.position= "none",
        axis.title.x=element_blank())
ggsave("output/base_loss_tPs.png", width = 10.16, height = 2)


```

# (distal more important thant proximal)
```{r}
