...
#ggsave("output/OL_seperate_basal.png", width = 6, height = 8)
...
test.pi <- pi.ol.dat %>% filter(Genotype == "4OL") %>% loess(formula = new_median ~ new_time, span = 0.75, degree = 2) 
...
pi.slope <- pi.slope %>% mutate(Genotype = factor(Genotype, levels = c("WT","PC","4OL","6OL", "8OL","10OL"))) 
...
oxi.slope <- oxi.slope %>% mutate(Genotype = factor(Genotype, levels = c("WT","PC","4OL","6OL", "8OL","10OL"))) 
...
levels.norescue <- c(WT = "wt",`2V` = "2V",`4V` = "4V", `6V` = "6V", `8V` = "8V")
...
```{r}
# Use 0min data from both H2O2 and -Pi treatment; 
data.TrunV <- data.TrunV.pi %>% bind_rows(data.TrunV.oxi ) %>%  
  select(Time, replicate = Replicate, genotype, median, Treatment) %>%
  mutate(Genotype = fct_recode(genotype, !!!levels)) %>% 
  mutate(new_median =median/1000) %>% 
  mutate(new_time = as.numeric(gsub("min","",Time)))%>% 
