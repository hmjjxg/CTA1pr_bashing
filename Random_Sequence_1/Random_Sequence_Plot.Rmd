...
pi.model %>% ggplot(aes(x = new_time, y = model, color = Genotype)) + reduced.timecourse + geom_line(aes(y = model)) + scale_color_manual(values= genotype_color, limits = names(genotype_color))
...
reg_info <- area %>% 
  mutate(pi_pcnt = pi.area/max(pi.area)) %>% 
  mutate(h2o2_pcnt = h2o2.area/max(h2o2.area))

reg_info %>% 
  ggplot(aes(x = pi_pcnt, y = h2o2_pcnt), color = Genotype) +
  geom_point(size = 3, fill = genotype_color) +
  scatter + 
  labs(x = "Pi (%)", y = "H2O2 (%)") +
  scale_color_manual(values = genotype_color, limits = names(genotype_color))
