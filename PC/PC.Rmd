...
names(files) <- c("Rep1-H2O2-Ctrl","Rep2-H2O2-Ctrl")
...
dat.ord <- map_dfr(files, ~read_csv(., na = c("", "NA", "N/A"), col_types = cols()), .id = "Replicate") %>% 
  filter(`X Parameter` == "BL1-H", is.na(`Y Parameter`), Sample != "beads", Count > 100) %>% 
...
names(tr_files) <- c("Rep1-H2O2-Ctrl","Rep2-H2O2-Ctrl")
...
pi.dat <- data %>% 
  ...
  mutate(Genotype = fct_recode(genotype, `PC` = "wt", `CTA1-GFP` = "CTA1-GFP",  ScCTA1pr = "ScCTA1pr::CgCTA1pr", ScCTT1pr ="ScCTT1pr::CgCTA1pr" , `Tn_pool` = "Tn_pool")) %>% 
...
names(genotype_color) <- c("PC", "CTA1-GFP",  "ScCTA1pr", "ScCTT1pr", "Tn_pool")
...
#ggsave("output/Sc_catalase_pr_2mMH2O2.png", width = 4.2, height = 4)
```
